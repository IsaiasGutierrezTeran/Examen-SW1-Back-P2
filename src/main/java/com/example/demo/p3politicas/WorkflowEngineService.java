...
import com.example.demo.p4tramites.CompletarNodoRequest;
...
import com.example.demo.p4tramites.IniciarTramiteRequest;
import com.example.demo.p4tramites.RepositorioDocumental;
import com.example.demo.p4tramites.RepositorioDocumentalService;
...
import com.example.demo.p4tramites.Secuencia;
...
import com.example.demo.p8reportes.MetricaYCuelloService;
...
public class WorkflowEngineService {
    ...
    @Autowired private com.example.demo.p1seguridad.UsuarioRepository usuarioRepository;
    ...
    private Tramite avanzarDesde(Tramite tramite, NodoDiagrama nodoOrigen,
        ...
        return switch (nodoOrigen.getTipo()) {
            ...
            case "fork" -> {
                ...
                // Reparto: ramas del mismo depto van a funcionarios distintos (si hay).
                java.util.Set<String> funcionariosAsignados = new java.util.HashSet<>();
                ...
                for (FlujoTransicion t : transiciones) {
                    ...
                    String funcRama = elegirFuncionarioDelDepto(
                            rama.getDepartamentoId(), funcionariosAsignados);
                    if (funcRama != null) funcionariosAsignados.add(funcRama);
    ...
    private String elegirFuncionarioDelDepto(String departamentoId) {
        return elegirFuncionarioDelDepto(departamentoId, java.util.Collections.emptySet());
    }

    /**
     * Elige un funcionario activo del departamento. Si se pasan funcionarios ya
     * asignados (ramas paralelas), prefiere uno que aún no haya sido usado para
     * repartir la carga entre ramas del mismo depto; si todos están usados o solo
     * hay uno, reutiliza el primero disponible.
     */
    private String elegirFuncionarioDelDepto(String departamentoId, java.util.Set<String> excluir) {
        if (departamentoId == null) return null;
        List<Usuario> delDepto = usuarioRepository.findByTipo("funcionario").stream()
                .filter(u -> u.getDepartamentosIds() != null
                        && u.getDepartamentosIds().contains(departamentoId)
                        && u.isActivo())
                .toList();
        return delDepto.stream()
                .filter(u -> !excluir.contains(u.getId()))
                .findFirst()
                .or(() -> delDepto.stream().findFirst())
                .map(Usuario::getId)
                .orElse(null);
    }
