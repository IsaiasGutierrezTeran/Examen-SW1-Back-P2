    ...
    public Map<String, Object> resumenDashboard() {
        ...
        for (Tramite t : tramites) {
            String estado = t.getEstadoActual() != null
                    ? com.example.demo.p4tramites.EstadoTramite.from(t.getEstadoActual()).getValor()
                    : "Sin estado";
            porEstado.merge(estado, 1, Integer::sum);
            if (!com.example.demo.p4tramites.EstadoTramite.esFinalizado(t.getEstadoActual())) activos++;
        }
        ...
        for (Tramite t : tramites) {
            if (com.example.demo.p4tramites.EstadoTramite.esFinalizado(t.getEstadoActual())) continue;
